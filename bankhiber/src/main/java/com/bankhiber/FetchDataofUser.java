...
public class FetchDataofUser {
	...
	public static void main(String[] args) {
		...
		// Create CriteriaBuilder to build criteria queries
		CriteriaBuilder builder = session.getCriteriaBuilder();
		
		// Create CriteriaQuery to define the query structure
		...
		// Get all the IDs from the BankUser table
		...
		// Get the ID of the newly added data in the table
		String hql1 = "SELECT s.id FROM BankUser s ORDER BY s.id DESC";
		Query query1 = session.createQuery(hql1);
		query1.setMaxResults(1);
		Integer id2 = (Integer) query1.uniqueResult();
		System.out.print("Your user ID is: " + id2 + " ");
		
		BankUser newUser = new BankUser();
		// Set the properties of the new user object

		// Save the new user to the table
		session.save(newUser);

		// Access the ID of the newly added entity
		Integer id23 = newUser.getId();
